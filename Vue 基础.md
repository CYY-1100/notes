# 一、 核心原理与底层机制
这部分考察的是对Vue框架工作原理的理解深度。
## 响应式原理

### 题目: Vue2 和 Vue3 的响应式原理分别是什么？它们之间最核心的区别是什么？
参考答案: Vue2 使用 Object.defineProperty() 来劫持对象属性的 getter 和 setter，从而追踪依赖和派发更新。Vue3 则采用了 ES6 的 Proxy 对象。核心区别在于，defineProperty 只能劫持对象的已有属性，对新增或删除属性无能为力（需要 Vue.set 或 Vue.delete），且无法监听数组索引的变化。而 Proxy 是对整个对象的代理，可以完美地拦截对象的读取、赋值、删除等所有操作，解决了上述所有问题，代码也更简洁。
虚拟DOM (Virtual DOM)
