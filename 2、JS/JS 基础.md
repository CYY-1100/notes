# JS 基础
## `<script>` 的加载时机
- defer：延迟加载，等到 HTML 解析完成后再按顺序执行。
  1. HTML 解析器 defer，会请求下载脚本（并行于 HTML 解析），继续解析后续 HTML。
  2. 当整个文档解析完成，依次执行所有 defer 脚本
- async：异步加载，加载完成后立即执行（不保证顺序）。
  1. HTML 解析器 async，发起异步下载，继续解析 HTML
  2. 当脚本下载完成，立即中断 HTML 解析，执行该脚本。

## JS数据类型有哪些？有什么区别？String的底层是什么？
- 有哪些
  - 基本数据：undefined，null，Number，String，Boolean，BigInt，Symbol
  - 引用数据：Object
- 区别
  - 基本数据：按值比较；传递值的副本；由JavaScript引擎自动管理
  - 引用数据：按引用地址比较；传递引用的副本；需要垃圾回收机制处理
...
- 遍历对象原型链，和 constructor.prorotype 进行===比对
  - 对基本数据类型无效，总是返回 false
- 具体步骤
  - 获取右侧操作数（构造函数）的 prototype 属性
  - 获取左侧操作数（实例对象）的内部属性 [[Prototype]]
  - 从对象的原型开始，沿着原型链向上遍历
    - 如果当前原型与 Constructor.prototype 严格相等（===），则返回 true。
    - 否则，继续获取当前原型的原型（__proto__），重复比较。
    - 如果到达原型链顶端（即 null），仍未找到匹配项，则返回 false。
...
- 内部函数被外部引用时，会带队外部函数的执行上下文，变量不会被回收
...
## 导致this指向混乱原因
- 根本原因this的指向不取决于函数在哪里定义，而取决于函数如何被调用。
...
- 原型
  ...
  - 函数：prototype
- 怎么获取
  - Object.getPrototypeOf() 或 `实例.__proto__`
- 原型链：
