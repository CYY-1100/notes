...
getBoundingClientRect() 最推荐，性能较好，适合频繁调用
...
2.geputedtldo
计算后的样式
3.offset.width scrolt.width client.width
布局树中的信息
dom.getBoundingcttentrect
视觉尺寸
...
- 没有 super 绑定
除二义性的核心在于：
this词法绑定：消除了运行时动态绑定的不确定性
功能限制：不能作为构造函数、没有arguments对象等，强制明确函数用途
语法设计：通过简洁语法和强制规则，减少语法解析的歧义
词法作用域：始终基于定义时的上下文，而非调用时的上下文
